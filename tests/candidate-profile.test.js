...
const checks = [
  ...
  ['profile version 1.2.0', eligible.candidate_profile_version === '1.2.0'],
  ['compact scorer schema', scorer.schema === 'mark.candidate_for_scorer.v1'],
  ['compact scorer version', scorer.profile_version === '1.2.0'],
  ...
  ['gaps include FastAPI', profile.known_gaps.some((gap) => gap.name === 'FastAPI')],
  ['gaps include SQL', profile.known_gaps.some((gap) => gap.name === 'SQL and PostgreSQL')],
  ['salary is not a filter', profile.work_preferences.salary_filter_applied === false],
  ['remote is allowed from any location', profile.work_preferences.remote.allowed_from_any_location === true],
  ['remote location restrictions are not a filter', profile.work_preferences.remote.location_restrictions_are_not_a_filter === true],
